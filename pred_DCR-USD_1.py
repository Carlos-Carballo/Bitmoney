...
for label in df['label']:
    if (label == 'Cluster 0'):
        ...
        n_cluster[2] += 1
pos = {0:1,1:0,2:2}        
etiqueta = {0 : 'Venta', 1 : 'Compra', 2 : 'Incertodumbre'}
color = {0 : 'red',1 : 'green',2 : 'yellow'}

#fig, ax = plt.subplots()
#for name, group in groups:
#    if(name == 'Cluster 0'):
#        ax.plot(group.x, group.y, marker='o', linestyle='', ms=5, label='Venta', color = 'red')
#    elif(name == 'Cluster 1'):
...
model = tf.keras.models.load_model('CNN_DCR-USD_1.h5')
model.summary()
imagen = img()

imagen = np.asarray(imagen).reshape(-1,32,32,1)

etiqueta = ['Venta', 'Compra', 'Incertidumbre']

predictions = model.predict(imagen)
score = tf.nn.softmax(predictions[0])
#print(score)

print("\033[1m {} \033[0m con 95% de exactitud.".format(etiqueta[pos[np.argmax(score)]]))
